...
class InfluxDbWrapper(object):
    def __init__(self, db_name:str, db_host:str, db_port:int, db_username:str, db_pwd:str):
        ...
        try:
            ...
            self.client = InfluxDBClient(host=self.db_host, port=self.db_port, username=self.db_username, password=self.db_pwd, database=self.db_name)
...
async def main() -> None:
    ...
    if data_path.exists():
        ...
        # MAC addresses of your RAPT Pill(s) - in case you have more (This hasn't been actually tested but it should in theory work.)
        pill = RaptPill("Exmple Session Name", "78:E3:6D:29:19:16", 120, influx_database_details=influx_details)
        pill.start_session()
